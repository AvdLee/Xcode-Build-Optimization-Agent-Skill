...
- confidence notes if noise prevents a strong conclusion -- if any `summary.<type>.high_variance` is true in the benchmark artifact (script default: spread exceeds 10% of the median), say so explicitly rather than presenting noisy numbers as definitive improvements or regressions
